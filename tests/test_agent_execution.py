...
from backend.agents.chat import run_agent, AgentResponse, AgentDeps
...
async def test_run_agent_execution_with_null_related(db_session):
    ...
    mock_agent_run.output = mock_agent_response
    mock_agent_run.all_messages = lambda: []
    with patch('backend.agents.chat.agent.run', return_value=mock_agent_run):
