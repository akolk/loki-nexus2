...
def test_execute_query_with_rd_coords():
    ...
    tool.con.execute(
        "INSERT INTO test_coords VALUES (3, 155000, 700000)"
    )  # Invalid RD Y
    tool.con.execute(
        "INSERT INTO test_coords VALUES (4, 400000, 463000)"
    )  # Invalid RD X
    tool.con.execute(
        "INSERT INTO test_coords VALUES (5, 155000, 200000)"
    )  # Invalid RD Y
...
def test_run_data_query_spatial_transform():
    # Helper wrapper test for spatial transformation
    res_str = run_data_query("SELECT id, x, y, value FROM test_data")
    assert "wgs84_lon" in res_str
    assert "wgs84_lat" in res_str
    assert "Test Point" in res_str
