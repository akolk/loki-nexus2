...
def test_chat_flow(mock_run_agent):
    # Setup mock
    mock_run_agent.return_value = {"response": {"answer": "Mocked Agent Response", "related": [], "code": None, "disclaimer": None, "reasoning": None, "error": None}, "exec_result": None}
...
def test_job_scheduling():
    ...
    with patch("backend.api.chat.run_agent") as mock_run:
        mock_run.return_value = {"response": {"answer": "ack", "related": [], "code": None, "disclaimer": None, "reasoning": None, "error": None}, "exec_result": None}
        client.post(
            "/chat",
