...
class DataTool:
    ...
    def __init__(self, db_path: str = ":memory:", username: Optional[str] = None):
        ...
        # Cache the pyproj Transformer instance for performance
        self._transformer = pyproj.Transformer.from_crs("EPSG:28992", "EPSG:4326", always_xy=True)
    ...
    def _transform_coordinates(self, df) -> Any:
        ...
        if 'x' in df.columns and 'y' in df.columns:
            ...
            if mask.any():
                ...
                lon, lat = self._transformer.transform(x_num[mask].values, y_num[mask].values)
