...
def find_endpoint(query: str, source_type: str = "pdok", top_k: int = 1, filter_geojson: bool = False) -> str:
    """
    ...
    result = loop.run_until_complete(find_endpoint_async(query, source_type, top_k, filter_geojson))
    logger.info(result)
    return result


async def find_endpoint_async(query: str, source_type: str = "pdok", top_k: int = 1, filter_geojson: bool = False) -> str:
    """
...
def search_metadata(query: str, source_type: Optional[str] = None, limit: int = 10) -> List[dict]:
    """
...
async def search_metadata_async(query: str, source_type: Optional[str] = None, limit: int = 10) -> List[dict]:
    """
    ...
    try:
        ...
        if source_type:
            sql = text("""
                ...
                LIMIT :limit
            """)
            result = session.execute(sql, {"embedding_str": embedding_str, "source_type": source_type, "limit": limit})
        else:
            sql = text("""
                SELECT e.id, e.endpoint_url, e.title, e.description, e.api_type,
                       s.name as source_name, s.source_type,
                ...
                LIMIT :limit
            """)
            result = session.execute(sql, {"embedding_str": embedding_str, "limit": limit})
