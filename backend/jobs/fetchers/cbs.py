...
async def fetch_endpoint_metadata(client: httpx.AsyncClient, identifier: str) -> Optional[Dict[str, Any]]:
    """Fetch metadata for a specific CBS endpoint asynchronously."""
    try:
...
async def fetch_cbs_metadata() -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Pre-fetch existing MetadataEndpoint records into a dictionary to optimize database access
            ...
            for dataset in value:
                try:
                    ...
                    existing = existing_endpoints_dict.get(odata_url)
                    ...
                    if embedding is None:
                        logger.warning(f"Failed to generate embedding for {identifier}, using fallback")
                        embedding = None  # Let database column handle None
                    ...
                    if existing:
                        ...
                        # Add to our dictionary in case we see it again within the same batch
                        existing_endpoints_dict[odata_url] = endpoint
                        endpoints_added += 1
            ...
            # Batch commit outside of loop to improve throughput
            session.commit()
