...
async def fetch_cbs_metadata() -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Batch embeddings by chunking them
            chunk_size = 100
            for i in range(0, total_datasets, chunk_size):
                chunk = value[i:i + chunk_size]

                # Gather embedding texts for the chunk
                embedding_texts = []
                ...
                try:
                    ...
                    logger.warning(f"Failed to generate embeddings batch: {e}. Skipping chunk.")
                    continue

                for j, dataset in enumerate(chunk):
                    try:
                        ...
                        existing = endpoints_cache.get(odata_url)
                        ...
                        if existing:
                            ...
                            endpoints_cache[odata_url] = endpoint
                            endpoints_added += 1
