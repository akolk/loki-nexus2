...
async def fetch_ogc_api_info(client: httpx.AsyncClient, api_url: str) -> Dict[str, Any]:
    ...
    try:
        response = await client.get(f"{api_url}?f=json", timeout=10.0)
...
async def fetch_collections_metadata(
    client: httpx.AsyncClient, collections_url: str
) -> List[Dict[str, Any]]:
    """Fetch metadata for all collections in an OGC API endpoint."""
    collections = []

    try:
        response = await client.get(f"{collections_url}?f=json", timeout=15.0)
        response.raise_for_status()
        data = response.json()
...
async def fetch_pdok_metadata() -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(PDOK_INDEX_URL, timeout=30.0)
            response.raise_for_status()
            ...
            for api in collections:
                ...
                try:
                    ...
                    existing = endpoint_dict.get(api_url)
                    ...
                    if existing:
                        ...
                        endpoint_dict[api_url] = endpoint
                        endpoints_added += 1

                    session.commit()

                    if collections_url:
                        collections_data = await fetch_collections_metadata(
                        ...
                        for coll in collections_data:
                            try:
                                ...
                                coll_existing = endpoint_dict.get(coll_url)
                                ...
                                if coll_keywords_str:
                                    coll_embedding_text += (
                                        f" Keywords: {coll_keywords_str}"
                                    )

                                coll_embedding = await generate_embedding(
                                    coll_embedding_text
                                )
                                ...
                                if coll_existing:
                                    ...
                                    endpoint_dict[coll_url] = coll_endpoint
                            except Exception as coll_err:
                                logger.warning(
                                    f"Error processing collection {coll.get('id')} in {title}: {coll_err}"
                                )
                                continue
