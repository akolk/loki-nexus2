...
async def fetch_ogc_api_info(api_url: str, client: httpx.AsyncClient) -> Dict[str, Any]:
...
async def fetch_collections_metadata(collections_url: str, client: httpx.AsyncClient) -> List[Dict[str, Any]]:
...
async def fetch_pdok_metadata() -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Pre-fetch all existing endpoints for O(1) lookup
            existing_endpoints = session.exec(
                select(MetadataEndpoint).where(MetadataEndpoint.source_id == source.id)
            ).all()
            endpoints_dict = {ep.endpoint_url: ep for ep in existing_endpoints}
            ...
            for api in collections:
                ...
                try:
                    ...
                    api_info = await fetch_ogc_api_info(api_url, client)
                    collections_url = api_info.get("collections_url")

                    existing = endpoints_dict.get(api_url)
                    ...
                    if existing:
                        ...
                        endpoints_dict[api_url] = endpoint
                        endpoints_added += 1

                    if collections_url:
                        collections_data = await fetch_collections_metadata(collections_url, client)
                        ...
                        for coll in collections_data:
                            ...
                            coll_existing = endpoints_dict.get(coll_url)
                            ...
                            if coll_existing:
                                ...
                                endpoints_dict[coll_url] = coll_endpoint

                except Exception as e:
                    logger.warning(f"Error processing API {title}: {e}")
                    continue

            # Move session.commit() outside the loops to optimize DB throughput
            session.commit()
