...
from backend.jobs.embeddings import generate_embedding, generate_embeddings_batch
...
async def fetch_ogc_api_info(client: httpx.AsyncClient, api_url: str) -> Dict[str, Any]:
    """Fetch OGC API info to find tileserver and collections URLs asynchronously."""
...
async def fetch_collections_metadata(client: httpx.AsyncClient, collections_url: str) -> List[Dict[str, Any]]:
    """Fetch metadata for all collections in an OGC API endpoint asynchronously."""
    collections = []
...
async def fetch_pdok_metadata() -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Pre-fetch existing MetadataEndpoint records into a dictionary
            existing_endpoints = session.exec(
                select(MetadataEndpoint).where(MetadataEndpoint.source_id == source.id)
            ).all()
            existing_endpoints_dict = {ep.endpoint_url: ep for ep in existing_endpoints}
            ...
            for api in collections:
                ...
                try:
                    ...
                    if embedding is None:
                        logger.warning(f"Failed to generate embedding for {api_url}, using fallback")
                        embedding = None  # Use None for fallback
                    ...
                    if collections_url:
                        ...
                        # Batch embedding generation for collections
                        valid_colls = []
                        embedding_texts = []
                        ...
                        if embedding_texts:
                            try:
                                embeddings_batch = await generate_embeddings_batch(embedding_texts)
                            except Exception as e:
                                logger.error(f"Error generating batch embeddings for collections in {title}: {e}")
                                # Fallback: empty lists to match length, will process as None below
                                embeddings_batch = [None] * len(embedding_texts)

                        for idx, coll in enumerate(valid_colls):
                            coll_url = coll.get("features_url", "")
                            coll_id = coll.get("id", "")
                            coll_title = coll.get("title", "")
                            coll_desc = coll.get("description", "")
                            ...
                            coll_embedding = embeddings_batch[idx] if idx < len(embeddings_batch) else None
            ...
            # Batch commit at the end instead of per-API
            session.commit()
